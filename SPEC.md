...
- `src/actor_game.rs`: isolated actor-oriented rewrite surface with
  ...
  per-player scores/stocks, high-score state, the effective source-wave
  profile, driver-owned source RNG, and actor snapshots with velocity/facing
  plus hostile-projectile source metadata into clean
...
- Normal `cargo run` and the explicit `--actor-live` alias open an interactive
  ...
  plus the first restored reserve batch when real actor wave-clear/wave-start
  and reserve-activation logic reaches them.
...
- `src/actor_game.rs` is an isolated actor-oriented rewrite slice. It exposes a
  ...
  reporting plus assisted next-wave and reserve-activation summaries for
  source/reserve counts, restored family counts, damage, fire cadence, and
  ...
  their actor-owned fraction state during active motion. The actor driver owns
