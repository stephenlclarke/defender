...
- Verified ROM image views exist for fixed main-CPU ROM, banked program ROMs,
  ...
  and can snapshot source-labeled CMOS and main-RAM fields. A main-board
  address classifier exists for RAM, banked I/O,
  ...
  step/read/format/mutate the source `AUDITG` / `MSGAUD` table rows, but CPU IRQ
  scheduling, LED segment side effects, exact Williams power-on RAM contents,
  `AUDITG` live text transfer/debounce timing or `CROM0` diagnostics after that
...
- CMOS layout, ROM default bytes, 4-bit cell writes, `CLRAUD`/`CMINIT` visible
  ...
  mutations, and red-label packed byte/word helper behavior are modeled, but
  CMOS persistence, `AUDITG` live text transfer/debounce timing, high-score
