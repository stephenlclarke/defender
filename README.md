...
`TIEST` player-relative squad placement and alternating X velocity before
entering the source bomber runtime. Enemy projectile evidence now distinguishes
source `FBOUT` fireballs from source `BMBOUT` bomber bomb shells and carries
source `BMBP1` shell descriptor fields for the standalone mine/source-shell
fixture. Enemy-projectile/player collision uses the source `BMBP1` 2x3
footprint while the direct runtime projectile renderer keeps the existing 4x6
bomb sprite.
