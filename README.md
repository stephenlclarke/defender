...
- <https://github.com/mwenge/defender>: Motorola 6809 assembly language for the
  ...
  astronaut restore layout plus the `defb6.src` `PRBST` / `PRBKIL` probe-start
  and pod-burst paths now embedded in `src/game.rs`, with the extracted text
  block kept in `assets/arcade/arcade-rules.txt` as a checked-in reference
  copy.
