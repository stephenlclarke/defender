...
mod public_api_tests {
    ...
    fn clean_runtime_and_oracle_use_quarantined_adapters() {
        ...
        let fidelity_manifest_rs = include_str!("fidelity_manifest.rs");
        assert!(fidelity_manifest_rs.contains("pub(crate) struct FidelityScenario"));
        assert!(fidelity_manifest_rs.contains("pub(crate) fn scenarios("));
        assert!(fidelity_manifest_rs.contains("pub(crate) fn expanded_input_text("));
        assert!(fidelity_manifest_rs.contains("crate::legacy_fidelity::TraceScenario"));
        assert!(fidelity_manifest_rs.contains("crate::legacy_fidelity::trace_scenarios()"));
        assert!(fidelity_manifest_rs.contains("crate::legacy_fidelity::expanded_trace_input_text"));
        ...
        assert!(fidelity_scenarios_rs.contains("crate::fidelity_manifest::scenarios()"));
        ...
        let game_smoke_rs = include_str!("game_smoke.rs");
        assert!(game_smoke_rs.contains("pub(crate) fn run("));
        assert!(game_smoke_rs.contains("pub(crate) fn smoke_report("));
        ...
        assert!(game_smoke_rs.contains("plan.frame_plan.scene_projection_upload_byte_len()"));
        ...
        assert!(game_smoke_rs.contains("upload.instance_count()"));
        assert!(game_smoke_rs.contains("plan.sprite_buffer_uploads"));
        assert!(game_smoke_rs.contains("uploads.quad_vertices.byte_len"));
        assert!(game_smoke_rs.contains("uploads.quad_indices.byte_len"));
        assert!(game_smoke_rs.contains("uploads.instances.byte_len"));
        assert!(game_smoke_rs.contains("plan.sprite_render_pass"));
        ...
        assert!(game_smoke_rs.contains("layout.bind_group_count()"));
        assert!(game_smoke_rs.contains("layout.binding_entry_count()"));
        assert!(game_smoke_rs.contains("plan.sprite_render_pipeline_descriptor"));
        assert!(game_smoke_rs.contains("descriptor.layout_bind_group_count()"));
        ...
        assert!(game_smoke_rs.contains("sprite_pipeline_layout_bind_groups"));
        assert!(game_smoke_rs.contains("sprite_pipeline_layout_binding_entries"));
        ...
        for forbidden in [
            ...
            assert!(
                !game_smoke_rs.contains(forbidden),
                "clean game smoke must not import legacy smoke dependency {forbidden}"
            );
        }
