...
const DATA_DIR_ENV: &str = "DEFENDER_DATA_DIR";
const REPO_NAME: &str = "defender";
...
mod tests {
    ...
    fn key_value_override_replaces_only_listed_keys() {
        ...
        assert!(merged.contains("safe_fall_height=2"));
        assert!(merged.contains("safe_fall_score=999"));
        assert!(merged.contains("human_catch_score=500"));
    }

    #[test]
    fn missing_override_file_uses_embedded_text() {
        ...
        assert_eq!(loaded, "safe_fall_height=2\nsafe_fall_score=250\n");
    }
}
