...
use crate::{
    ...
    SceneDrawPlan, SpriteId, SpriteRenderPassEncoderPlan, SpriteRenderPipelineDescriptorPlan,
...
pub(crate) struct GameSmokeReport {
    ...
    pub(crate) sprite_resource_bind_groups: usize,
    pub(crate) sprite_resource_binding_entries: usize,
    ...
    pub(crate) sprite_encoder_set_pipeline_commands: usize,
    pub(crate) sprite_encoder_set_bind_group_commands: usize,
    pub(crate) sprite_encoder_set_vertex_buffer_commands: usize,
    pub(crate) sprite_encoder_set_index_buffer_commands: usize,
...
impl GameSmokeReport {
    pub(crate) fn validate(&self) -> anyhow::Result<()> {
        ...
        if self.sprite_resource_bind_groups
            != self.frames as usize * SpriteResourceBindingPlan::BIND_GROUP_COUNT
        {
            bail!("clean game smoke did not produce sprite resource bind groups for every frame");
        }
        if self.sprite_resource_binding_entries
            != self.frames as usize * SpriteResourceBindingPlan::BINDING_ENTRY_COUNT
        {
            bail!(
                "clean game smoke did not produce sprite resource binding entries for every frame"
            );
        }
        ...
        if self.sprite_pipeline_layout_bind_groups != self.sprite_resource_bind_groups {
            bail!(
                "clean game smoke sprite pipeline layout bind groups did not match resource bind groups"
            );
        }
        if self.sprite_pipeline_layout_binding_entries != self.sprite_resource_binding_entries {
            bail!(
                "clean game smoke sprite pipeline layout binding entries did not match resource binding entries"
            );
        }
        ...
        if self.sprite_encoder_set_pipeline_commands
            != self.frames as usize * SpriteRenderPassEncoderPlan::SET_PIPELINE_COMMAND_COUNT
        {
            bail!(
                "clean game smoke did not produce sprite render-pass set-pipeline commands for every frame"
            );
        }
        if self.sprite_encoder_set_bind_group_commands != self.sprite_pipeline_layout_bind_groups {
            bail!(
                "clean game smoke sprite render-pass bind group commands did not match pipeline layout bind groups"
            );
        }
        if self.sprite_encoder_set_vertex_buffer_commands
            != self.sprite_render_pipeline_descriptor_vertex_buffers
        {
            bail!(
                "clean game smoke sprite render-pass vertex buffer commands did not match pipeline descriptor vertex buffers"
            );
        }
        if self.sprite_encoder_set_index_buffer_commands
            != self.frames as usize * SpriteRenderPassEncoderPlan::SET_INDEX_BUFFER_COMMAND_COUNT
        {
            bail!(
                "clean game smoke did not produce sprite render-pass index buffer commands for every frame"
            );
        }
    ...
    pub(crate) fn to_text(&self) -> String {
        ...
        format!(
            ...
            self.sprite_resource_bind_groups,
            self.sprite_resource_binding_entries,
            ...
            self.sprite_encoder_set_pipeline_commands,
            self.sprite_encoder_set_bind_group_commands,
            self.sprite_encoder_set_vertex_buffer_commands,
            self.sprite_encoder_set_index_buffer_commands,
...
fn observe_frame(
    ...
    if let Some(bindings) = &plan.sprite_resource_bindings {
        ...
        report.sprite_resource_bind_groups = report
            .sprite_resource_bind_groups
            .saturating_add(bindings.bind_group_count());
        report.sprite_resource_binding_entries = report
            .sprite_resource_binding_entries
            .saturating_add(bindings.binding_entry_count());
    ...
    if let Some(layout) = &plan.sprite_pipeline_layout {
        report.sprite_pipeline_layout_frames =
            report.sprite_pipeline_layout_frames.saturating_add(1);
        report.sprite_pipeline_layout_bind_groups = report
            .sprite_pipeline_layout_bind_groups
            .saturating_add(layout.bind_group_count());
        report.sprite_pipeline_layout_binding_entries = report
            .sprite_pipeline_layout_binding_entries
    ...
    if let Some(encoder) = &plan.sprite_render_pass_encoder {
        ...
        report.sprite_encoder_set_pipeline_commands = report
            .sprite_encoder_set_pipeline_commands
            .saturating_add(encoder.set_pipeline_command_count());
        report.sprite_encoder_set_bind_group_commands = report
            .sprite_encoder_set_bind_group_commands
            .saturating_add(encoder.set_bind_group_command_count());
        report.sprite_encoder_set_vertex_buffer_commands = report
            .sprite_encoder_set_vertex_buffer_commands
            .saturating_add(encoder.set_vertex_buffer_command_count());
        report.sprite_encoder_set_index_buffer_commands = report
            .sprite_encoder_set_index_buffer_commands
            .saturating_add(encoder.set_index_buffer_command_count());
...
mod tests {
    ...
    fn smoke_report_exercises_clean_game_and_native_draw_plans() {
        ...
        assert_eq!(
            report.sprite_resource_bind_groups,
            report.frames as usize * SpriteResourceBindingPlan::BIND_GROUP_COUNT
        );
        assert_eq!(
            report.sprite_resource_binding_entries,
            report.frames as usize * SpriteResourceBindingPlan::BINDING_ENTRY_COUNT
        );
        assert_eq!(report.sprite_pipeline_layout_frames, report.frames);
        assert_eq!(
            report.sprite_pipeline_layout_bind_groups,
            report.frames as usize * SpritePipelineLayoutPlan::BIND_GROUP_COUNT
        );
        assert_eq!(
            report.sprite_pipeline_layout_binding_entries,
            report.frames as usize * SpritePipelineLayoutPlan::BINDING_ENTRY_COUNT
        );
        assert_eq!(
            report.sprite_pipeline_layout_bind_groups,
            report.sprite_resource_bind_groups
        ...
        assert_eq!(
            report.sprite_encoder_set_pipeline_commands,
            report.frames as usize * SpriteRenderPassEncoderPlan::SET_PIPELINE_COMMAND_COUNT
        );
        assert_eq!(
            report.sprite_encoder_set_bind_group_commands,
            report.sprite_pipeline_layout_bind_groups
        );
        assert_eq!(
            report.sprite_encoder_set_vertex_buffer_commands,
            report.sprite_render_pipeline_descriptor_vertex_buffers
        );
        assert_eq!(
            report.sprite_encoder_set_index_buffer_commands,
            report.frames as usize * SpriteRenderPassEncoderPlan::SET_INDEX_BUFFER_COMMAND_COUNT
        );
    ...
    fn smoke_report_validates_gpu_resource_evidence() {
        ...
        let mut report = valid_report();
        report.sprite_resource_bind_groups = report.sprite_resource_bind_groups.saturating_sub(1);

        let error = report
            .validate()
            .expect_err("missing resource bind group should fail");

        assert_eq!(
            error.to_string(),
            "clean game smoke did not produce sprite resource bind groups for every frame"
        );

        let mut report = valid_report();
        report.sprite_resource_binding_entries =
            report.sprite_resource_binding_entries.saturating_sub(1);

        let error = report
            .validate()
            .expect_err("missing resource binding entry should fail");
        ...
        let mut report = valid_report();
        report.sprite_pipeline_layout_bind_groups =
            report.sprite_pipeline_layout_bind_groups.saturating_sub(1);

        let error = report
            .validate()
            .expect_err("pipeline layout bind group mismatch should fail");
        ...
        let error = report
            .validate()
            .expect_err("pipeline layout binding entry mismatch should fail");
        ...
        let error = report
            .validate()
            .expect_err("missing set-pipeline encoder command should fail");
        ...
        let error = report
            .validate()
            .expect_err("bind group encoder command mismatch should fail");
        ...
        let error = report
            .validate()
            .expect_err("vertex buffer encoder command mismatch should fail");
        ...
        let error = report
            .validate()
            .expect_err("missing set-index-buffer encoder command should fail");
    ...
    fn smoke_report_formats_current_cli_output() {
        let report = GameSmokeReport {
            ...
            sprite_resource_bind_groups: 4,
            sprite_resource_binding_entries: 6,
            ...
            sprite_encoder_set_pipeline_commands: 2,
            sprite_encoder_set_bind_group_commands: 4,
            sprite_encoder_set_vertex_buffer_commands: 4,
            sprite_encoder_set_index_buffer_commands: 2,
    ...
    fn valid_report() -> GameSmokeReport {
        GameSmokeReport {
            ...
            sprite_resource_bind_groups: 6,
            sprite_resource_binding_entries: 9,
            ...
            sprite_encoder_set_pipeline_commands: 3,
            sprite_encoder_set_bind_group_commands: 6,
            sprite_encoder_set_vertex_buffer_commands: 6,
            sprite_encoder_set_index_buffer_commands: 3,
